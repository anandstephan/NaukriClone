...
router.get("/appliedJob", requireLogin, (req, res) => {
  if (req.user.mode == "candidate") {
    let id = req.user.id;
    let sql =
      "select jobs.name,jobs.title,jobs.description,jobs.duration from jobs INNER JOIN applied_jobs as app_jobs ON jobs.id=app_jobs.id where app_jobs.candidate_id=" +
      id;
    console.log(sql);
...
router.post("/applyingjob", requireLogin, (req, res) => {
  if (req.user.mode == "candidate") {
